...
namespace ImageToGrayscale
{
    ...
    public static class BitmapUtils
    {
        ...
        /// <returns></returns>
        public static Bitmap GrayImage(Bitmap bmp, GrayAlgorithmType type)
        {
            int height = bmp.Height;
            int width = bmp.Width;
            Bitmap newbmp = new Bitmap(width, height);
            ...
            for(int x = 0; x < width; x++)
            {
                for(int y = 0; y < height; y++)
                {
                    ...
                    switch(type)
                    {
                        case GrayAlgorithmType.Average: //平均值
                            Result = ((r + g + b) / 3);
                            break;
                        case GrayAlgorithmType.Maximum: //最大值
                            Result = r > g ? r : g;
                            Result = Result > b ? Result : b;
                            break;
                        case GrayAlgorithmType.WeightedAverage: //加權平均值
        ...
        public static Bitmap RemoveFrame(Bitmap bitmap, PaddingFrame padding, Color color)
        {
            int height = bitmap.Height;
            int width = bitmap.Width;
